...
const test1 = async () => {
    ...
    const query = `SELECT carton, leaveDate, customer, printCode, soldTo,
                    ...
                    vas, left(customer, 9) as scust, cartonType
                    FROM japan2.orders`;
    ...
    let dyVasUnits = groupBy(data, ['wdate', 'vas'], ['units'], [])
    ...
    let custAll = groupBy(data, ['scust', 'vas'], ['units'], ['carton','wdate','shipTo','sku'])
    let columns1 = [
        ...
        { key: "sku_dcnt", name: "SKUs", idx: 7, type: "number" },
        { key: "vas", name: "Vas", idx: 8, type: "string" },
    ]
    addWS(wb1, 'custsum', columns1, custAll);

    let custKeyAll = groupBy(keyOrds, ['scust', 'vas'], ['units'], ['carton','wdate','shipTo','sku'])
...
let cust20200324 = groupBy(Ords20200324, ['scust', 'vas'], ['units'], ['carton','wdate','shipTo','sku'])
